...
export async function initContract() {
  ...
  window.accountWrap = await near.account("wrap.testnet")
  window.accountWrap = await new Contract(window.walletConnection.account(), 'wrap.testnet', {
    changeMethods: ['ft_transfer_call', 'near_deposit'],
});
  // Initializing our contract APIs by contract name and configuration

  window.ftContract = await new Contract(window.walletConnection.account(), 'wrap.testnet', {
      viewMethods: [ 
        'ft_balance_of',
      ],
      changeMethods: ['ft_transfer_call', 'near_deposit'],
  ...
  window.streamContract = await new Contract(window.walletConnection.account(), 'streaming-r-v2.dcversus.testnet', {
    viewMethods: [ 
      'get_account_outgoing_streams', 'get_account_incoming_streams'
    ],
    changeMethods: ['ft_transfer_call', 'near_deposit', 'pause_stream', 'start_stream', 'stop_stream', 'withdraw'],
});
